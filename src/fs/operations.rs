...
/// Implementation of FileSystemOperations.
/// Concrete implementation of FileSystemOperations.
...
impl FileSystemOperations for FileSystemOperationsImpl {
    ...
    async fn create_dir(&self, path: &Path) -> Result<(), TaskError> {
        let normalized = normalize_path(path);
        retry_operation(|| {
            fs::create_dir_all(&normalized).map_err(TaskError::from)
        }).await
    }
    ...
    async fn delete_file(&self, path: &Path) -> Result<(), TaskError> {
        let normalized = normalize_path(path);
        retry_operation(|| {
            fs::remove_file(&normalized).map_err(TaskError::from)
        }).await
    }
    ...
    async fn delete_dir(&self, path: &Path) -> Result<(), TaskError> {
        let normalized = normalize_path(path);
        retry_operation(|| {
            fs::remove_dir_all(&normalized).map_err(TaskError::from)
        }).await
    }
    ...
    async fn rename_file(&self, from: &Path, to: &Path) -> Result<(), TaskError> {
        ...
        retry_operation(|| {
            fs::rename(&from_norm, &to_norm).map_err(TaskError::from)
        }).await
    }
    ...
    async fn file_metadata(&self, path: &Path) -> Result<fs::Metadata, TaskError> {
        let normalized = normalize_path(path);
        retry_operation(|| {
            fs::metadata(&normalized).map_err(TaskError::from)
        }).await
    }
}
